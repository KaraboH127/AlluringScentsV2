...
import type { Collection, Fragrance, SizeOption } from "../../types/site";
...
export function FragranceHero({
  fragrance, collection, selectedSize, onSizeChange, quantity,
        ...
        <div className="space-y-1">
          {isOnSale ? (
            <div className="flex items-end gap-3">
              <p className="text-3xl accent-gold site-heading">{formatCurrency(salePrice!)}</p>
              <p className="text-lg text-muted line-through mb-0.5">{formatCurrency(originalPrice)}</p>
              <span className="mb-1 bg-red-500 text-white text-[10px] font-bold uppercase tracking-widest px-2 py-1">
                {saleLabel}
              </span>
            </div>
          ) : (
            <p className="text-3xl accent-gold site-heading">{formatCurrency(originalPrice)}</p>
          )}
        </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {(["10ml", "50ml", "100ml"] as SizeOption[]).map((s) => {
                ...
                return (
                  ...
                  >
                    {s}
                    {onSale && <span className="absolute -top-1.5 -right-1.5 w-2 h-2 bg-red-500 rounded-full" />}
                  </button>
                );
              })}
            </div>
          ...
          {stockLoading ? (
            ...
            <>
              <p className={`text-xs tracking-wide ${
                isOutOfStock ? "text-red-400" : isLowStock ? "text-yellow-400" : "text-green-500"
              }`}>
                {isOutOfStock ? "Out of stock for this size" : isLowStock ? `Only ${currentStock} left` : "In stock"}
              </p>

              {!isOutOfStock && (
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-[0.14em] text-muted">Quantity</p>
                  <QuantitySelector value={quantity} onChange={onQuantityChange} max={currentStock} />
                </div>
              )}
